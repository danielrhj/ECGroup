...
com.formatDateTime = function (value) {
    return utils.formatDate(value, 'yyyy-MM-dd hh:mm');
};
...
com.formatMoney = function (value) {
    ...
    return sign + utils.formatNumber(Math.abs(value), '#,##0.000000');
};
...
com.formatMoney3 = function (value) {
...
com.refreshTab = function (url,newTitle) {
    parent.wrapper.tabRefreshNew.apply(this, arguments);
}

com.setCurrentTabTitle = function (title) {
    parent.wrapper.setCurrentTabTitle.apply(this, arguments);
};
...
com.editGridViewModel = function (grid) {
    ...
    this.addnew0 = function (rowData) {
        if (self.ended()) {
            if (Object.prototype.toString.call(rowData) != '[object Object]') rowData = {};
            rowData = $.extend({ _isnew: true }, rowData);
            grid.datagrid('insertRow', { index: 0, row: rowData });
            self.editIndex = 0; //grid.datagrid('getRows').length - 1;
            grid.datagrid('selectRow', self.editIndex);
            self.begin(self.editIndex, rowData);
        }
    };
...
com.exporter = function (opt) {
    ...
    this.title = function (field, title) {
        var arrfield = field.split(",");
        var arrtitle = title.split(",");

        for (var i = 0; i < arrfield.length; i++)
        { self.params.titles[0].push({ field: arrfield[i], title: arrtitle[i] }); }
        return self;
    };
...
com.openFile = function (filePath) {
    var strFullPath = window.document.location.href;//當前頁面完整URL
    var strPath = window.document.location.pathname;//當前頁面相對URL

    var pos = strFullPath.indexOf(strPath);
    var prePath = strFullPath.substring(0, pos);

    window.open(prePath + filePath);
};

com.setFirstPageWhenSearchGrid = function (grid) {
...
com.myValid = function (sourceList, checkValue, options) {
    options = $.extend({
        required: true,
        msgTip: '必填項'
    },
    ...
    var msg = '';
    if (options.required && (!checkValue)) { return options.msgTip + ':不能為空.'; }
    $.each(sourceList, function (i, row) {
        if (row.value!='' && (row.value == checkValue)) {            
            msg = ''; return msg;
        }        
    });
    ...
    if (msg == '')
        return '';
    else { return options.msgTip + ':選項值無效'; }
    
};

com.mergeGrid = function (grid, fields) {
    if (!fields) {
...
com.mergeGridCopy = function (grid, fields, copyFields) {
    ...
    for (i; i < rows.length; i++) {
        ...
        for (p; p < copyFields.length; p++) {
            var fieldMore = copyFields[p];     
            temp[fieldMore] = temp[fields[0]];            
        }
    }
