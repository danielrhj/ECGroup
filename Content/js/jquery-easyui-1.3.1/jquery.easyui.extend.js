    ...
    $.extend($.fn.datagrid.defaults.editors, {
        ...
        checkbox: {//调用名称
            init: function (container, options) {   //container 用于装载编辑器 options,提供编辑器初始参数                       
                ...
                //这里我把一个 checkbox类型的输入控件添加到容器container中需要渲染成easyu提供的控件，需要时用传入options,我这里如果需要一个combobox，就可以 这样调用 input.combobox(options);
                return input;
            },
            destroy: function (target) {

            },
            getValue: function (target) { //datagrid 结束编辑模式，通过该方法返回编辑最终值,这里如果用户勾选中checkbox返回1否则返回0
                return $(target).prop("checked") ? 1 : 0;
            },
            setValue: function (target, value) {  //datagrid 进入编辑器模式，通过该方法为编辑赋值,我传入value 为0或者1，若用户传入1则勾选编辑器
                if (value)
                    $(target).prop("checked", "checked");
            },
            resize: function (target, width) {      //列宽改变后调整编辑器宽度
                var input = $(target);
                if ($.boxModel == true) {
