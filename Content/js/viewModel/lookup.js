...
function getOptions() {
    ...
    var valueWidth = options.grid.columns[0].length ? 80 : Math.floor(w * 0.2);
    var textWidth = options.grid.columns[0].length ? 80 : Math.floor(w * 0.2);
    ...
    //根据data.lookupType添加其他栏位
    if (options.lookupType == 'supplier') {
        options.grid.columns[0].push({ title: 'Contact', field: 'Contact', align: 'left', sortable: true, width: 150 });
        options.grid.columns[0].push({ title: 'Tel', field: 'Tel', align: 'left', sortable: true, width: 150 });
    }
