...
wrapper.init = function () {
    com.ajax({ type: 'GET', url: 'GetUserMenu', success: wrapper.initMenu });//此处使用wrapper.initMenu加载菜单
...
wrapper.tabContextMenu = function (e, title) {
    $('#closeMenu').menu('show', { left: e.pageX, top: e.pageY }); //注意这个closeMenu是index.cshtml里面的div,然后再加菜单
...
wrapper.changePassword = function () {
    com.dialog({
        ...
        viewModel: function (w) {
            w.find("[name=UserCode]").val($.cookie('UserCode'));
            w.find("#pwd_confirm").click(function () {
                post = { key: $("[name=UserCode]").val(), oldPWD: $("[type=password]")[0].value, newPWD: $("[type=password]")[1].value, newCFMPWD: $("[type=password]")[2].value };
                if (post.newCFMPWD.length == 0 || post.newPWD.length == 0)
                { com.message('ERROR', '新密碼不可為空.'); }
                else if (post.newCFMPWD != post.newPWD)
                { com.message('ERROR', '新密碼與確認密碼不同.'); }
                else
                {
                    com.ajax({
                        type: 'GET',
...
wrapper.initMenu = function (d) {
    ...
    $('body').data('menulist', d);  //將資料d放在JQuery緩存中,并分配一個id=menulist
    var visibleMenu = $.grep(d, function (row) { return row.IsVisible; }); //在JQuery緩存中篩選可見的菜單列表
    ...
    $("#home").html(wrapper.createFrame("Main"));
    //$("#home").html(wrapper.createFrame("Default"));
};
...
wrapper.tabRefreshNew = function (url,newTitle) {
    var $tab = $("#tabs");
    //var currentTab = $tab.tabs('getTab', oldTitle);
    var currentTab = $('#tabs').tabs('getSelected');
    var src = $(currentTab.panel('options').content).attr('src');
    if (typeof src === 'string') src = url;
    $tab.tabs('update', { tab: currentTab, options: { title: newTitle } }); //, content: wrapper.createFrame(src)
...
wrapper.setCurrentTabTitle = function (title) {
...
wrapper.menuTree = function (menus) {
    var settings = {
        data: { key: { name: "MenuName", url: "URL" } },
        callback: {
