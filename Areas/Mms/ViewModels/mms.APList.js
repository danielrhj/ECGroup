...
mms.APList = function (data) {
    ...
    this.grid = {
        ...
        onLoadSuccess: function (data) {    //根據checkbox綁定的值禁用checkbox
            ...
            com.mergeGridCopy(self.grid, ['RcvNo'], ['SuppName', 'Currency', 'PayAmount', 'APStatus', 'DueDate', 'BatchNo', 'BatchDate', 'Remarks', 'RemitNo', 'RemitDate', 'PayDetail']);
        },
    ...
    this.CreateBatchNoClick = function () {
        ...
        if (checkedItems.length == 0) { com.message('warning', self.resx.noneSelect); return false; }

        var array = [];
        $.each(checkedItems, function (index, item) {
            array.push(item.APID);
        });

        if (array.length == 0) { com.message('warning', self.resx.noneSelect);return false;  }
    ...
    this.deleteClick = function () {
        ...
        $.each(row, function (index, item) {
            array.push(item.APID); arrayTypeCode.push(item.RcvNo);
        });
        var str = array.join(',');
...
mms.Detail = function (data) {
    ...
    this.grid.queryParams({ APID: self.form.APID() });
    ...
    this.deleteClick = function () {
        ...
        $.each(row, function (index, item) {
            array.push(item.SNO); arrayTypeCode.push(item.PayNo);
        });
        var str = array.join(',');
        com.message('confirm', "确定要删除 " + arrayTypeCode.join(',') + " 的資料吗？", function (b) {
            if (b) {
                com.ajax({
                    ...
                    success: function () {
                        com.message('success', self.resx.deleteSuccess);
                        self.grid.queryParams({ APID: self.form.APID() });
                    }
                });
            }
        });
...
var openURLReceiving = function (data0, data1) {
    com.openTab('入庫单--' + data1, "/mms/receiving/Edit/" + data0);
};

var openURLDetail = function (data) {
    var param = { BizID: data, BizTable: 'APSub' };
    mms.com.ARAPDetail(param);
};

var SaveAPOnceClear = function (edit) {
    var apid = edit;
    ...
    var APStatus = $(editor[1].target).combobox('getValue');
    var PayNo = $(editor[2].target).val();
    var PayDate = $(editor[3].target).datebox('getValue');
    var Remarks = $(editor[4].target).val();

    if (!PayNo || !PayDate) { com.message('error', '一次结清时必须录入付款单据号码和实际付款日期!'); }
