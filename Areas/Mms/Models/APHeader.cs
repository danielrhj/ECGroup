...
{
    [Module("EC")]
    public class APListService : ServiceBase<APList>
    {
        public static string strSP = "DG_APHeader_sp";
        internal IList<APList> GetList(ParamQuery pQuery)
        {
            string strSQL = pQuery.GetSQL();
            List<APList> myList = db.Select<APList>(strSQL).QueryMany();

            return myList;
        }

        public static List<dynamic> getAPStatusList()
        {
            ParamSP ps = new ParamSP().Name(strSP);
            ParamSPData psd = ps.GetData();
            ps.Parameter("ActionType", "GetAPStatusList");
            List<dynamic> resultA = new APListService().GetDynamicList(ps);
            resultA.Insert(0, new {value="",text="全部" });
            return resultA; 
        }
       
        internal static string ExportAPListExcel(ParamSP ps)
        {
            string fileTemp = Zephyr.Utils.ZHttp.RootPhysicalPath + @"Content\Template\APCheckList.xlsx";
            DataSet ds = new APListService().StoredProcedureDS(ps);
            ...
            for (int i = 0; i < dtSub.Rows.Count; i++)
            {
                ...
                myExcel.myExcelST.Cells[refCell].Offset(0, 1).Value = drSub["BuyNo"].ToString();
                myExcel.myExcelST.Cells[refCell].Offset(0, 2).Value = drSub["SuppPN"].ToString();
                myExcel.myExcelST.Cells[refCell].Offset(0, 3).Value = drSub["RcvDate"].ToString();
                myExcel.myExcelST.Cells[refCell].Offset(0, 4).Value = drSub["RcvNo"].ToString();
                ...
                myExcel.myExcelST.Cells[cellPrice].Value = decimal.Parse(drSub["RcvPrice"].ToString());
